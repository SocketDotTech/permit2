...
contract SignatureTransferTest is
    Test,
    PermitSignature,
    TokenProvider,
    GasSnapshot
{
    ...
    function testPermitTransferFromRandomNonceAndAmount(
        uint256 nonce,
        uint128 amount
    ) public {
        ...
        uint256 startBalanceTo = token0.balanceOf(address2);
        ISignatureTransfer.SignatureTransferDetails
            memory transferDetails = getTransferDetails(address2, amount);
    ...
    function testPermitTransferSpendLessThanFull(
        uint256 nonce,
        uint128 amount
    ) public {
        token0.mint(address(from), amount);
    ...
    function testPermitBatchTransferFrom() public {
        uint256 nonce = 0;
        address[] memory tokens = AddressBuilder.fill(1, address(token0)).push(
            address(token1)
        );
        ISignatureTransfer.PermitBatchTransferFrom
            memory permit = defaultERC20PermitMultiple(tokens, nonce);
        bytes memory sig = getPermitBatchTransferSignature(
            permit,
            fromPrivateKey,
            DOMAIN_SEPARATOR
        );

        address[] memory to = AddressBuilder.fill(1, address(address2)).push(
            address(address0)
        );
        ISignatureTransfer.SignatureTransferDetails[]
    ...
    function testPermitBatchMultiPermitSingleTransfer() public {
        ...
        address[] memory to = AddressBuilder.fill(1, address(address0)).push(
            address(address0)
        );
        ISignatureTransfer.SignatureTransferDetails[]
            memory toAmountPairs = StructBuilder.fillSigTransferDetails(
    ...
    function testPermitBatchTransferMultiAddr() public {
        ...
        address[] memory to = AddressBuilder.fill(1, address(this)).push(
            address2
        );
        ISignatureTransfer.SignatureTransferDetails[]
            memory toAmountPairs = StructBuilder.fillSigTransferDetails(
    ...
    function testPermitBatchTransferSingleRecipientManyTokens() public {
        ...
        assertEq(
            ...
            startBalanceFrom0 - 10 * defaultAmount
        );
        assertEq(
            token0.balanceOf(address(this)),
            startBalanceTo0 + 10 * defaultAmount
        );
    }
    ...
    function testGasMultiplePermitBatchTransferFrom() public {
        uint256 nonce = 0;
        address[] memory tokens = AddressBuilder
            .fill(1, address(token0))
            .push(address(token1))
            .push(address(token1));
        ISignatureTransfer.PermitBatchTransferFrom
            memory permit = defaultERC20PermitMultiple(tokens, nonce);
        ...
        address[] memory to = AddressBuilder.fill(2, address(address2)).push(
            address(this)
        );
        ISignatureTransfer.SignatureTransferDetails[]
            memory toAmountPairs = StructBuilder.fillSigTransferDetails(
        ...
        assertEq(
            ...
            startBalanceToThis1 + defaultAmount
        );
    }

    function testPermitBatchTransferFromTypedWitness() public {
        uint256 nonce = 0;
        MockWitness memory witnessData = MockWitness(
            10000000,
            address(5),
            true
        );
    ...
    function testInvalidateUnorderedNonces() public {
        ISignatureTransfer.PermitTransferFrom
            memory permit = defaultERC20PermitTransfer(address(token0), 0);
        bytes memory sig = getPermitTransferSignature(
            permit,
            fromPrivateKey,
            DOMAIN_SEPARATOR
    ...
    function testPermitTransferFromTypedWitness() public {
        ...
        bytes32 witness = keccak256(abi.encode(witnessData));
        ISignatureTransfer.PermitTransferFrom
            memory permit = defaultERC20PermitWitnessTransfer(
                address(token0),
                nonce
            );
