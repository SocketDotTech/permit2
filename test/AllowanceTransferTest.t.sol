...
contract AllowanceTransferTest is
    Test,
    TokenProvider,
    PermitSignature,
    GasSnapshot
{
    ...
    function testSetAllowanceBatchDifferentNonces() public {
        ...
        bytes memory sig1 = getPermitBatchSignature(
            permitBatch,
            fromPrivateKey,
            DOMAIN_SEPARATOR
        );
        ...
        (amount, expiration, nonce) = permit2.allowance(
            from,
            address(token0),
            address(this)
        );
        ...
        (uint160 amount1, uint48 expiration1, uint48 nonce1) = permit2
            .allowance(from, address(token1), address(this));
    ...
    function testSetAllowanceBatchDirtyWrite() public {
        ...
        assertEq(nonce, 2);
        (uint160 amount1, uint48 expiration1, uint48 nonce1) = permit2
            .allowance(fromDirty, address(token1), address(this));
        assertEq(amount1, defaultAmount);
        assertEq(expiration1, defaultExpiration);
        assertEq(nonce1, 2);
    }
    ...
    function testSetAllowanceDeadlinePassed() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(SignatureExpired.selector, sigDeadline)
        );
        permit2.permit(from, permit, sig);
        snapEnd();
    }
    ...
    function testMaxAllowance() public {
        ...
        (uint160 startAllowedAmount0, , ) = permit2.allowance(
            from,
            address(token0),
            address(this)
        );
        ...
        (uint160 endAllowedAmount0, , ) = permit2.allowance(
            from,
            address(token0),
            address(this)
        );
    ...
    function testInvalidateMultipleNonces() public {
        ...
        permit2.permit(from, permit, sig);
        (, , uint48 nonce1) = permit2.allowance(
            from,
            address(token0),
            address(this)
        );
        ...
        permit = defaultERC20PermitAllowance(
            address(token1),
            defaultAmount,
            defaultExpiration,
            nonce1
        ...
        (, , uint48 nonce2) = permit2.allowance(
            from,
            address(token0),
            address(this)
        );
    ...
    function testBatchTransferFromDifferentOwners() public {
        ...
        IAllowanceTransfer.PermitSingle
            memory permitDirty = defaultERC20PermitAllowance(
                address(token0),
                defaultAmount,
                defaultExpiration,
                dirtyNonce
            );
        bytes memory sigDirty = getPermitSignature(
            permitDirty,
            fromPrivateKeyDirty,
            DOMAIN_SEPARATOR
        );
        ...
        assertEq(amount, defaultAmount);
        (uint160 amount1, , ) = permit2.allowance(
            fromDirty,
            address(token0),
            address(this)
        );
    ...
    function testLockdown() public {
        ...
        IAllowanceTransfer.TokenSpenderPair[]
            memory approvals = new IAllowanceTransfer.TokenSpenderPair[](2);
        approvals[0] = IAllowanceTransfer.TokenSpenderPair(
            address(token0),
            address(this)
        );
        approvals[1] = IAllowanceTransfer.TokenSpenderPair(
            address(token1),
            address(this)
        );
        ...
        assertEq(expiration, defaultExpiration);
        assertEq(nonce, 1);
        (amount1, expiration1, nonce1) = permit2.allowance(
            from,
            address(token1),
            address(this)
        );
