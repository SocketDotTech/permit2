...
contract Permit2LibTest is Test, PermitSignature, GasSnapshot {
    ...
    ERC20 immutable weth9Mainnet =
        ERC20(payable(address(0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2)));
    ...
    MockNonPermitERC20 immutable nonPermitToken =
        new MockNonPermitERC20("Mock NonPermit Token", "MOCK", 18);
    MockFallbackERC20 immutable fallbackToken =
        new MockFallbackERC20("Mock Fallback Token", "MOCK", 18);
    MockPermitWithSmallDS immutable lessDSToken =
        new MockPermitWithSmallDS(
            "Mock Permit Token Small Domain Sep",
            "MOCK",
            18
        );
    MockPermitWithLargerDS immutable largerDSToken =
        new MockPermitWithLargerDS(
            "Mock Permit Token Larger Domain Sep",
            "MOCK",
            18
        );
    MockNonPermitNonERC20WithDS immutable largerNonStandardDSToken =
    ...
    constructor() {
        ...
        MockNonPermitERC20(address(weth9Mainnet)).mint(
            address(this),
            type(uint128).max
        );
    ...
    function testPermit2LargerDS() public {
        ...
        Permit2Lib.permit2(
            MockERC20(address(largerDSToken)),
            PK_OWNER,
            address(0xCAFE),
            1e18,
            block.timestamp,
    ...
    function testOZSafePermitPlusOZSafeTransferFrom() public {
        ...
        snapStart(
            "safePermit + safeTransferFrom with an EIP-2612 native token"
        );
    ...
    function testPermit2PlusTransferFrom2WithNonPermitFallback() public {
        ...
        snapStart(
            "permit2 + transferFrom2 with a non EIP-2612 native token with fallback"
        );
    ...
    function testSimplePermit2PlusTransferFrom2WithNonPermit() public {
        ...
        snapStart(
            "simplePermit2 + transferFrom2 with a non EIP-2612 native token"
        );
    ...
    function testPermit2DSLessToken() public {
        bool success = permit2Lib.testPermit2Code(
            MockERC20(address(lessDSToken))
        );
    ...
    function testPermit2DSMoreToken() public {
        bool success = permit2Lib.testPermit2Code(
            MockERC20(address(largerNonStandardDSToken))
        );
